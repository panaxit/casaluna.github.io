...
xo.listener.on(['domLoaded'], function ({ target }) {
	 target.querySelectorAll(`.association-ref`).toArray().map(ref => ref.scope).filter(scope => scope).forEach(scope => scope.dispatch("downloadCatalog"))
})
...
xo.listener.on(`input::input.money`, function ({ element: row, attribute, old, value }) {
	 debugger
})
...
xo.spaces["sitemap"] = "http://panax.io/sitemap";
xo.listener.on('fetch::#sitemap', function ({ document }) {
	 for (let menu of document.select(`//sitemap:menu[@title="Ventas"]`)) {
			menu.setAttribute("title", "Notas")
	 }
})
...
xo.listener.on(`filter::@*`, async function ({ event }) {
...
xo.listener.on(`fetch::#alertarCita`, function ({ document }) {
	 ...
	 if (!notificacions.documentElement) return;
	 let items = notificacions.createDocumentFragment();
	 for (let cita of document.select(`/*/row`)) {
			let row = notificacions.createElement("item");
			row.textContent = cita.getAttribute("mensaje")
			items.append(row)
	 }
	 notificacions.documentElement.replaceChildren(items)
})
