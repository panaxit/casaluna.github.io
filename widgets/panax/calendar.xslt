...
exclude-result-prefixes="#default xsl state data route"
>
...
/* opcional: que el contenido interno se acomode en columna */
.calendar .day{
  display:flex;
  ...
  flex: 1;
	...
	<xsl:template match="@*" mode="calendar:body-reservation.button.delete">
		...
		<div class="col-auto">
			<xsl:apply-templates mode="route:widget-button" select=".">
				<xsl:with-param name="dataset" select="$dataset"/>
			</xsl:apply-templates>
		</div>
	</xsl:template>

	<xsl:template match="*[@state:delete]/@*" mode="calendar:body-reservation" priority="10">
		<span class="badge_danger pill badge-danger-light mt-3">
			Eliminar<br/>Cita: <xsl:apply-templates mode="calendar:body-reservation-description" select="."/>
			<br/>
			<span class="btn btn-danger badge-delete p-1 badge-danger-light" xo-attribute="state:delete" onclick="scope.remove()">
