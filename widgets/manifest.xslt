...
>
	...
	<xsl:template name="widget:input" mode="widget" match="@*">
	...
	<xsl:template mode="widget" match="@*[key('widget',concat('radiogroup:',ancestor::*[key('entity',@xo:id)][1]/@xo:id,'::',name()))]">
		...
		<xsl:variable name="current" select="."/>
		<div class="btn-group bg-body form-control" role="group" style="position:relative;" xo-scope="{$current/../@xo:id}" xo-attribute="{name(.)}">
			<xsl:for-each select="$context[ancestor-or-self::xo:r[1]]">
				...
				<div class="form-check form-check-inline data-row" xo-scope="{../@xo:id}" xo-attribute="{name(.)}">
					<input class="form-check-input" type="radio" value="{$value}" id="{../@xo:id}_{position()}" xo-scope="{$current/../@xo:id}" xo-attribute="{name($current)}" name="{local-name($current)}" xo-swap="self::*">
